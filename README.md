...
*   **Triple Model Architecture**: 
    *   **MiniLM**: Powered by `all-MiniLM-L6-v2` for high-speed, lightweight inference.
    *   **MPNet**: Powered by `all-mpnet-base-v2` for balanced, high-quality semantic mapping.
    *   **Elite (DeBERTa)**: A high-precision Cross-Encoder (`stsb-roberta-large`) for maximum similarity accuracy.
*   **Advanced Linguistic Heuristics**: Enhanced with **NLI Cross-Encoders** for contradiction handling, negation detection, and antonym penalty logic.
*   **Tasks**:
...
Evaluate all models on the MRPC and STS datasets:
```bash
python3 main.py
```
